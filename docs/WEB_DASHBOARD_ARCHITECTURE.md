...
- 세션은 로컬 파일 기반 영속 저장소로 관리한다.
- 기본 전제는 `single-node + shared local volume` 이다.
- join ticket 사용 기록과 session store 는 `STATE_STORAGE_DIR` 아래 JSON 파일로 유지된다.
...
- local file store 는 재시작 복구용이며, 멀티 인스턴스 동시 쓰기를 보장하지 않는다.
