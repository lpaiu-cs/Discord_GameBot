...
1. Discord 사용자가 참가 버튼으로 로비에 들어간 뒤, 게임 시작 후 `/mafia dashboard` 또는 상태 메시지 버튼으로 입장 링크를 요청한다.
...
## Reconnect / dashboard 재발급 flow

- 기존 ephemeral 메시지는 source of truth 가 아니다.
- 사용자는 언제든 `/mafia dashboard` 로 새 join ticket 을 발급받을 수 있다.
