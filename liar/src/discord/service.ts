...
export class LiarDiscordService {
  ...
  async handleCommand(client: Client, interaction: ChatInputCommandInteraction): Promise<boolean> {
    ...
    await this.sendPublicMessage(
      client,
      game,
      "라이어게임 로비가 열렸습니다. 아래 버튼으로 참가하고, 방장은 카테고리를 고른 뒤 시작하세요.",
  ...
  private buildStatusPayload(game: LiarGame): { content: string; components: any[] } {
    ...
    if (game.phase === "lobby") {
      ...
      lines.push("방장은 카테고리를 선택한 뒤 `시작` 버튼을 누르세요.");
    }
  ...
  private buildStatusComponents(game: LiarGame): any[] {
    ...
    if (game.phase === "lobby") {
      controlRow.addComponents(
        ...
        new ButtonBuilder().setCustomId(`liar:start:${game.id}`).setLabel("시작").setStyle(ButtonStyle.Success).setDisabled(game.playerCount < 4),
  ...
  private buildResultAnnouncement(game: LiarGame, result: LiarResult): string {
    ...
    return [title, result.reason, wordLine, liarLine].filter((line): line is string => Boolean(line)).join("\n");
  }
