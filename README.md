...
- 현재 저장소는 `single-node`, `file-backed` 저장소를 사용합니다.
- join ticket 사용 기록, 웹 세션, 게임 상태는 `STATE_STORAGE_DIR` 아래 JSON 파일로 유지됩니다.
- 같은 호스트와 같은 볼륨을 유지하는 한 프로세스 재시작 뒤에도 게임과 세션을 복구합니다.
- 다만 멀티 인스턴스 동시 쓰기나 분산 락까지 해결한 구조는 아니므로, 다중 노드 수평 확장은 별도 저장소 계층이 필요합니다.
